...
import { Bell, BriefcaseBusiness, Building2, CalendarClock, ChartNoAxesColumnIncreasing, ChevronDown, CircleUserRound, Columns3, FileText, FolderOpen, Home, LogOut, Menu, Settings, Users, Workflow, X } from "lucide-react";
...
export function DashboardShell({ children }: { children: ReactNode }) {
  ...
  const handleSignOut = () => void signOut().then(() => router.replace("/login"));
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-20 items-center justify-between border-b border-slate-100 bg-white/90 px-5 backdrop-blur sm:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <button onClick={() => setMobileMenuOpen(true)} className="rounded-xl border border-slate-100 p-2.5 text-slate-500 lg:hidden">
              <Menu className="h-5 w-5" />
            </button>
            <div className="min-w-0">
              <p className="truncate text-sm text-slate-400">Real Estate Lead Management CRM</p>
              <p className="truncate font-semibold text-slate-900">Welcome, {user.name}</p>
            </div>
          </div>
