...
export interface SiteVisit { id: string; companyId: string; leadId: string; projectId: string; assignedTo?: string; visitDate: string; status: "Scheduled" | "Done" | "Cancelled"; notes?: string; createdAt: string; }
export interface Booking { id: string; companyId: string; leadId: string; projectId: string; unitId?: string; bookingDate: string; amount: string; status: "Pending" | "Confirmed" | "Cancelled"; createdAt: string; }
...
export const roleLabels: Record<Role, string> = { super_admin: "Super Admin", builder_admin: "Builder Admin", sales: "Sales Executive", broker: "Channel Partner", customer: "Customer" };
